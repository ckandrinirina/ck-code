...
**Do NOT use the `Skill` tool inside this procedure.** It resolves against
the global plugin registry (`superpowers:*`, `frontend-design:*`, etc.),
not the project-local files generated by `/ck-code:team` — a name collision
would silently load an unrelated plugin skill, and no match would error
