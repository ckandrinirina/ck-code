...
| `/ck-code:plan`           | Create epics, single-dispatch stories (each S/M so one `build`/`parallel-build` agent finishes it in a pass — larger work is split at a natural seam), and a roadmap                                                                                                             | spec file                                   | `tasks/YYYY-MM-DD_<project>/` + `tasks/FEATURE_INDEX.md` rows   |
