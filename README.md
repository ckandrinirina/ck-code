...
| `/ck-code:plan` | Create epics ordered demo-first (the app runs from epic 01, backend epics replace the UI's fixture seams), single-dispatch S/M stories, a mandatory final Integration & E2E epic, and a roadmap; `--quick [brief] [--epic NN]` adds one small story to an existing epic | spec file | `tasks/YYYY-MM-DD_<slug>/` (stories carry frontmatter) |
