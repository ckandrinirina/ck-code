...
| `/ck-code:plan` | Create epics ordered demo-first in three layers (the whole surface runs over fixtures first, one epic replaces the seams with the real backend, one epic runs Integration & E2E — more epics only for a hard boundary), single-dispatch S/M stories every one of which a human verifies through the surface rather than an API client, and a roadmap; `--quick [brief] [--epic NN]` adds one small story to an existing epic | spec file | `tasks/YYYY-MM-DD_<slug>/` (stories carry frontmatter) |
