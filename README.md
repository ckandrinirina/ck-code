...
| `/ck-code:doc-optimizer`  | Migrate legacy layer docs into per-feature docs, scaffold docs for features missing one, and prune/dedupe to cut tokens read during build/fix                                                                                                | `migrate` / `sync` / `optimize`   | feature-scoped `docs/architecture/` + `FEATURE_INDEX` `Docs`  |
