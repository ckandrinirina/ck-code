...
description: >
  ...
  dispatches parallel sub-agents each invoking /ck-code:build, then runs QA and conflict analysis.
...
Each sub-agent invokes `/ck-code:build` on its story in isolation. Ends with QA validation
...
Launching N agents in parallel:
  ...
  Story 02-06  (L)  →  claude-opus-4-7    →  branch story/02-06
  Story 03-01  (S)  →  claude-sonnet-4-7 1M token  →  branch story/03-01
...
# Check: binary exists at engine/build/Release/<your-binary>
```
...
Then run a final QA on the merged main (TypeScript + tests) to catch any cross-branch integration issues.
If all checks pass, proceed to **Phase 7: Worktree Cleanup**.

**If user picks 2:** print worktree paths and stop. Worktrees stay intact for manual review.
After user finishes reviewing and merging manually, remind them to run Phase 7.
...
**Goal:** Remove all agent worktrees after a successful merge. Always run this after merging.
...
For each worktree under `.claude/worktrees/agent-*`, remove it:

```bash
git worktree remove -f -f /path/to/.claude/worktrees/agent-XXXXXXXX
```

Use `-f -f` (double force) because agent worktrees are locked by default.
...
Run `git worktree list` again and confirm only the main worktree remains.
Print:
...
- **Always delete** agent worktrees after merging — use `git worktree remove -f -f` then `git worktree prune`
...
- **Run final QA on merged main** before cleanup — cross-branch integration issues only appear after all merges
