...
Pick a tier by story `Size:`, never a hardcoded versioned model ID:
...
2. **Latest-by-tier (default):** pick the latest model in the current Claude family —
   ...
   (e.g. `claude-sonnet-4-6`), `advanced` → top-tier (e.g. `claude-opus-4-7`),
   `advanced-extended-context` → top-tier long-context (e.g. `claude-opus-4-7[1m]`).
   Examples are illustrative — prefer newer IDs known to the session; if unsure, use the
   model the orchestrator itself runs (at least advanced-tier).
3. **Confirm before launch:** print the resolved Size → Tier → Model table in the
   announce step (3.2) so the operator can override a mis-resolution.
