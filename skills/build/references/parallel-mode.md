...
Detail for `SKILL.md` § PARALLEL MODE. Read **only** when two or more stories are in scope.
The orchestrator decides, verifies, and merges; it never builds, tests, or reads source.
...
`"${CLAUDE_PLUGIN_ROOT}/scripts/ck-index.sh" tasks/<Plan>` first if it is missing or lacks
...
**One story in scope is never orchestrated** — hand it to SKILL.md Phase 1.3 and run
Phases 1–8 inline. Orchestration begins at two.
...
clear high-reasoning signal (novel algorithm, concurrency/distributed correctness, intricate
state machine, security- or perf-critical path, non-obvious architecture); use **`fast`
(Haiku)** only for trivial mechanical changes. `size` reflects scope, not difficulty, so it
never escalates on its own. Honor `CK_MODEL_FAST|BALANCED|ADVANCED` env overrides. Print the
resolved Story → Complexity → Tier → Model line in the launch announce so a mis-resolution
is catchable before work starts.
...
- **◐ incomplete** — real diff but criteria still unchecked (the agent ran out of budget).
  Not failed. **Resume the same agent** with `SendMessage` ([agent-prompts.md](agent-prompts.md))
  — its worktree and context are intact. Re-run this gate; cap **2 resume rounds**. Still
...
No manifest match → ask once and reuse. Mark any QA-failing story **BLOCKED from merge** and
keep its branch. Merge-eligible = ✓ complete + `QA: PASS` + conflict-free.
...
3. **Re-dispatch blocked/failed stories** — fresh `Agent` call, new worktree, for empty or
   errored stories only (◐ incomplete is resumed at P5, not here)

**Option 1** — merge each eligible branch into `$TARGET` in P6's order:
...
Then **regenerate the indexes once** on the target branch — each merged worktree carries only
its own story frontmatter at `done` (sub-agents never touch the shared views); the generator
rebuilds them from that frontmatter with no cell-editing and no sole-writer hazard:
...
`$TARGET` in the main checkout (the union of the merged stories' commands, de-duplicated) — a
...
**Manual gate (SKILL.md 8.5, once per wave).** Ask the operator to exercise the merged work
on `$TARGET` in the main checkout (`AskUserQuestion` `PASS / ISSUES`) — the only place every
merged story sits together with real dependencies. On `ISSUES`, dispatch one `Agent` into the
