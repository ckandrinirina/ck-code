...
model: <tier from parallel-mode.md § P4 — balanced default, advanced only on a high-reasoning signal>
isolation: worktree
...
prompt: |
  ...
  2. Follow it completely — it owns TDD, SOLID, QA, and committing. Because this prompt
     ...
     no `ck-index.sh`, no manual-test gate, no ship. **Commit after every TDD cycle /
  ...
  - Edit only files relevant to this story, and only this story's own frontmatter. Do NOT
    ...
    ck-index.sh — the orchestrator regenerates them once after merge.
