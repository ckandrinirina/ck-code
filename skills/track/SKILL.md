...
For each plan folder, Read `tasks/<slug>/STORIES_INDEX.md`. The table carries every
column needed: `Epic`, `ID`, `Title`, `Status`, `Size`, `Blocked by`, `File`.
...
with `"${CLAUDE_PLUGIN_ROOT}/scripts/ck-index.sh"` if missing).

### 1.3 Resolve readiness

Mark each row per **The Ready rule** in
[`../guide/SKILL.md`](../guide/SKILL.md#the-ready-rule) (single source of truth): a row
is `ready` if `status: todo` and every `Blocked by` id resolves to `done`, **or** if
`status: bug` (a triaged bug is always actionable — `build` implements its recorded fix
in Bug-Fix Mode; surface it distinctly with 🐛). Everything else is `blocked`.
