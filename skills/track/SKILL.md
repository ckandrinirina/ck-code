...
carries every column needed: `Epic`, `ID`, `Title`, `Status`, `Delivery`, `Size`,
`Blocked by`, `File`.
...
**Report the two axes separately.** `Status` says the work is finished; `Delivery` (`—`,
`PR #<n>`, `MERGED`) says how far it reached toward the trunk branch
([`../../references/data-model.md`](../../references/data-model.md#two-axes-status-is-work-delivery-is-integration)).
A `DONE` row with an empty Delivery is finished but **not shipped** — say so rather than
calling it done, since that is the exact gap this view exists to close. The cell is a cache
refreshed by `ck-project sync`, so this skill stays read-only and offline: never call `gh`
to re-check a PR here.
